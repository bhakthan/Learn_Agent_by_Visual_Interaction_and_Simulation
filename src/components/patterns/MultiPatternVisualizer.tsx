...
import { ArrowsCounterClockwise, Play, Stop, ArrowsHorizontal } from '@phosphor-icons/react'
...
const MultiPatternVisualizer = ({ initialPatterns, useAdvancedVisualizer = true }: MultiPatternVisualizerProps) => {
  ...
  const togglePatternSelection = useCallback((patternId: string) => {
    setSelectedPatternIds(current => {
      ...
      // If we already have 2 patterns, replace the last one
      if (current.length >= 2) {
      ...
      {showPatternSelector && (
        <Card className="mb-4">
          <CardHeader className="pb-2">
            ...
            <CardDescription>Choose up to 2 patterns to compare side by side</CardDescription>
          </CardHeader>
          <CardContent>
